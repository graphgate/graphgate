...
#[test]
fn test_provides_directive_with_fragments() {
    // Load and parse the test schema
    let schema_str = fs::read_to_string("tests/provides_fragments_test.graphql").expect("Failed to read schema file");
    let schema_doc = parse_schema(&schema_str).expect("Failed to parse schema");
    let schema = ComposedSchema::new(schema_doc);

    // Create a query that should use the @provides directive with fragment spreads
    let query = r#"
    {
      products {
        id
    ...
    match plan {
        RootNode::Query(plan_node) => {
            ...
            // because the "reviews" field on Product provides the author fields via fragment spread
